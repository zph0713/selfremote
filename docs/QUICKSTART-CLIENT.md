...
## 7. 免 root 用法：`sr socks`（本地 SOCKS5 / HTTP 代理）

不建虚拟网卡、不动系统路由表、不需要管理员权限：TCP 在进程内由用户态网络栈
终结，IP 包直接进隧道，只有显式走这个代理的程序受影响。

```bash
sr socks -c client-macbook.srkey -l 127.0.0.1:1080
sr socks -c client-macbook.srkey -l 127.0.0.1:1080 -dns 192.168.2.1   # 顺便用站点内 DNS 解析域名
```

```bash
curl --socks5-hostname 127.0.0.1:1080 http://192.168.1.50:5000/
ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:1080 %h %p' user@192.168.1.50
curl -p -x http://127.0.0.1:1080 http://192.168.1.50:5000/          # HTTP CONNECT 方式
# 浏览器：SOCKS5 代理 127.0.0.1:1080，勾选「远程 DNS」
```

说明：

- 只支持 **TCP**（SOCKS5 CONNECT）。UDP（ping、mDNS、游戏、QUIC 网页）用不了——
  需要这些就用整段路由模式 `sr client`
- `-dns <站点内 DNS>` 让域名解析也走隧道（不会把内网名字泄露给公网 DNS）；
  不填则只能访问 IP
- 目标仍受服务端 ACL 约束：只有该设备被授权的站点网段能通，其它目标会被丢弃
- 默认只监听 `127.0.0.1`。改成 `0.0.0.0` 等于把隧道入口开放给同网段所有人，慎用
